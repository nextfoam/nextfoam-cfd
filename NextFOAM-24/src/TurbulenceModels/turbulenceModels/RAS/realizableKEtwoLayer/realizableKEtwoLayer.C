...
)
{
    ...
    (
        epsilon_.mesh().data::template getOrDefault<bool>
    ...
    tmp<scalarField> Ap = eqn.D();

    // Calculate the sum-mag off-diagonal from the interior faces
    scalarField sumOff(Ap().size(), Zero);
    eqn.sumMagOffDiag(sumOff);

    // Scale off-diagonal and source using lambda
    eqn.lduMatrix::operator*=(lambda.field());
    eqn.diag() /= lambda.field();
    eqn.source() *= lambda.field();
    ...
    S += (D -D0)*epsilon_.primitiveField();
    S += (1 - lambda)*Ap*es;
}
