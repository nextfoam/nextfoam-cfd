...
{
    const dictionary& solutionDict = fvSolution(runTime_);

    nCorrPIMPLE_ = 
    (
        solutionDict.subDict("PIMPLE").getOrDefault<label>
        (
            "nOuterCorrectors", 
            1
        )
...
bool Foam::multiRegionPimpleControl::criteriaSatisfied()
{
    ...
    forAll(regions_, regioni)
    {
        ...
        const dictionary& solverDict = fvmesh->solverPerformanceDict();
...
bool Foam::multiRegionPimpleControl::loop()
{
    ...
    if (corr_ == nCorrPIMPLE_ + 1)
    {
		forAll(regions_, regioni)
        {
            ...
            fvmesh->data::remove("finalIteration");
        }
    ...
    if (converged_ || criteriaSatisfied())
    {
        ...
        else
        {
            ...
            forAll(regions_, regioni)
            {
                ...
                fvmesh->data::add("finalIteration", true);
            }
