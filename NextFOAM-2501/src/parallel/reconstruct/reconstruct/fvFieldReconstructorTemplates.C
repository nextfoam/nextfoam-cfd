...
)
{
    ...
    for (const IOobject& io : fieldObjects)
    {
        if (io.isHeaderClass<fieldType>())
        {
            ...
            {
                tmp<GeometricField<Type, fvPatchField, volMesh>> trcF
                (
                    reconstructVolumeField<Type>(io)
                );
                GeometricField<Type, fvPatchField, volMesh>& rcF = trcF.ref();

                auto& bf = rcF.boundaryField();

                bool writeBoundaryField(false);

                forAll(bf, patchi)
                {
                    const word& polyPatchType
                    (
                        bf[patchi].patch().type()
                    );
                    ...
                    (
                        polyPatchType == "patch"
                     || polyPatchType == "wall"
                     || polyPatchType == "mappedWall"
                    )
                    {
                        //if (!isA<calculatedFvPatchField<Type>>(bf[patchi]))
                        if (bf[patchi].type() != "calculated")
                        {
                            writeBoundaryField = true;
                        }
                    }
                }

                if (writeBoundaryField)
                {
                    rcF.readOpt() = IOobject::MUST_READ;
                    rcF.writeOpt() = IOobject::AUTO_WRITE;
                    rcF.writeBoundaryField(true); // temporary fix
                }

                rcF.write();
            } // by Gill

            //reconstructVolumeField<Type>(io)().write(); // by Gill
            ++nReconstructed_;
        }
    }
