...
:
    ...
    w_(1.1),
    LambdaC_(p_.primitiveField().size(), 0.0),
    LambdaV_(p_.primitiveField().size(), 0.0),
    spectralRadiiInv_
    (
        IOobject
        (
...
void spectralRadius::update()
{
    ...
    forAll(owner, facej)
    {
        LambdaC_[owner[facej]] += spectralRadiiInv_[facej];
        LambdaC_[neighbour[facej]] += spectralRadiiInv_[facej];

        LambdaV_[owner[facej]] += spectralRadiiVis_[facej];
        LambdaV_[neighbour[facej]] += spectralRadiiVis_[facej];
    }
    ...
    forAll(mesh_.boundary(), patchi)
    {
        ...
        {
            LambdaC_[pFaceCells[facej]] += pSpectralRadiiInv[facej];
            LambdaV_[pFaceCells[facej]] += pSpectralRadiiVis[facej];
        }
    }

    LambdaC_ = LambdaC_/w_;
    //LambdaC_ *= 0.5;
    LambdaV_ /=mesh_.V();
}
