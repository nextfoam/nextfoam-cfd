...
Foam::tmp<Foam::scalarField> Foam::temperatureCoupledBase::qAnisoCorr
(
...
) const
{
    ...
    switch (method_)
    {
        case mtFluidThermo:
        {
            ...
            {
                const auto* ptr =
                    mesh.cfindObject<volSymmTensorField>("Anikappa");
                ...
                if (ptr)
                {
                    ...
                    const vectorField nKappa(n & kappa);        

                    const vectorField& gradTBf
                    (
                        gradT().boundaryField()[patchi]
                    );

                    return -((nKappa - n*(nKappa & n)) & gradTBf);
                }
            }
        ...
        case mtSolidThermo:
        {
            ...
            {
                const auto* ptr =
                    mesh.cfindObject<volSymmTensorField>("Anialpha");
                ...
                if (ptr)
                {
                    ...
                    const symmTensorField kappa
                    (
                        alphaAni*thermo.Cp(pp, Tp, patchi)
                    );
