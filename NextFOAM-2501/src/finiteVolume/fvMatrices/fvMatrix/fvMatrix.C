...
:
    ...
    centralCoeffPtr_(nullptr),
    centralCoeffbyVPtr_(nullptr)
{
    DebugInFunction
...
Foam::fvMatrix<Type>::~fvMatrix()
{
    ...
    centralCoeffPtr_.clear();
    centralCoeffbyVPtr_.clear();
    subMatrices_.clear();
}
...
template<class Type>
void Foam::fvMatrix<Type>::updateCoeffs() // by Gill
{
    if 
    (
        psi_.mesh().objectRegistry::template foundObject<volScalarField>("UAp")
     && psi_.name() == "U"
     && psi_.dimensions() == dimVelocity
    )
    {
        volScalarField& UAp
        (
            ...
            (
                psi_.mesh().objectRegistry::template lookupObject
                <
                    volScalarField
                >("UAp")
            )
        ...
        UAp.primitiveFieldRef() = cmptMag(this->D());
        UAp.correctBoundaryConditions();
    }
}
...
template<class Type>
Foam::tmp<Foam::volScalarField> Foam::fvMatrix<Type>::updateA() const
{
    ...
    (
        ...
        (
            ...
            psi_.mesh(),
            dimensions_/psi_.dimensions(),
            extrapolatedCalculatedFvPatchScalarField::typeName
        )
    );

    tA.ref().primitiveFieldRef() = D();
...
template<class Type>
Foam::tmp<Foam::volScalarField> Foam::fvMatrix<Type>::updateAbyV
(
...
) const
{
    ...
    tAbyV.ref() = 1.0/VbyA;
    tAbyV.ref().correctBoundaryConditions(); 
...
template<class Type>
Foam::tmp<Foam::volScalarField> Foam::fvMatrix<Type>::VbyA
(
    bool updateAll
) const
{
    tmp<volScalarField> tVbyA
    (
        new volScalarField
        (
            IOobject
            ...
            psi_.mesh(),
            psi_.dimensions()*dimVol/dimensions_,
            extrapolatedCalculatedFvPatchScalarField::typeName
        )
    );

    tVbyA.ref().primitiveFieldRef() = psi_.mesh().V()/D();
    tVbyA.ref().correctBoundaryConditions();

    centralCoeffPtr_.reset(updateA().ptr());

    if (updateAll)
    {
        centralCoeffbyVPtr_.reset(updateAbyV(tVbyA()).ptr());
    }
...
template<class Type>
const Foam::volScalarField& Foam::fvMatrix<Type>::AbyV() const
{
    return centralCoeffbyVPtr_();
}
