...
#include "VKLimitedGrad.H"
#include "gaussGrad.H"

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

makeFvGradScheme(VKLimitedGrad)

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

template<class Type>
void Foam::fv::VKLimitedGrad<Type>::limitGradient
(
...
<
    ...
    >
>
Foam::fv::VKLimitedGrad<Type>::calcGrad
(
...
) const
{
    ...
    scalarField deltah = mesh.V();

    if(mesh.nGeometricD() == 2)
    {
        deltah = sqrt(deltah/mesh.bounds().span()[2]);
    }
    else
    {
        deltah = pow(deltah, 1.0/3);
    }
    ...
    forAll(bsf, patchi)
    {
        ...
        {
            const vectorField delta(psf.patch().delta());
