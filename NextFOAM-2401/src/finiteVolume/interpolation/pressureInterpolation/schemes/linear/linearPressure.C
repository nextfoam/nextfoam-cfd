...
Foam::tmp<Foam::surfaceScalarField> Foam::linearPressure::correction
(
...
) const
{
    ...
    (
        ...
        (
            IOobject
            (
                "linearPressure::correction(" + vf.name() + ')',
                mesh_.time().timeName(),
                mesh_,
                IOobject::NO_READ,
                IOobject::NO_WRITE,
    ...
    const surfaceScalarField& pIrFs = fvOptions_.porousInteriorFaces();
    ...
    forAll(bSfCorr, patchi)
    {
        ...
        const scalarField& pPIrFs = pIrFs.boundaryField()[patchi];
...
namespace Foam
{
    makePressureInterpolationScheme(linearPressure)
}
