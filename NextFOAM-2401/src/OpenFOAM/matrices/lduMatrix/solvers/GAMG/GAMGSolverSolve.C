...
) const
{
    ...
    (
        matrix().mesh().hasDb()
     && matrix().mesh().thisDb().foundObject<scalarField>("Umag")
     && (
    ...
    {   
        solveScalarField Atemp(psi.size());
 
        matrix_.Amul
        (
            Atemp,
            matrix().mesh().thisDb().lookupObject<scalarField>("Umag"),
            interfaceBouCoeffs_,
            interfaces_,
            cmpt
        );

        normFactor = gSumMag(Atemp, matrix().mesh().comm());

        if (normFactor < solverPerformance::small_)
        {
            normFactor =
    ...
    {
        normFactor = this->normFactor(psi, tsource(), Apsi, finestCorrection);
    }
