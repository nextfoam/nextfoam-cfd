...
Scalar ScalarRead(const char* buf)
{
    ...
    parsing::errorType err = parsing::checkConversion(buf, endptr);

    // Ignore "underflow" error
    // "overflow" error will be handled later
    if (err == parsing::errorType::RANGE)
        err = parsing::errorType::NONE;

    // Handle "overflow" error
    if (parsed < -ScalarVGREAT || parsed > ScalarVGREAT)
      err = parsing::errorType::RANGE;
...
bool ScalarRead(const char* buf, Scalar& val)
{
    ...
    // Ignore "underflow" error
    // "overflow" error was checked before
    if (err == parsing::errorType::RANGE
        || err == parsing::errorType::NONE)
        return true;
