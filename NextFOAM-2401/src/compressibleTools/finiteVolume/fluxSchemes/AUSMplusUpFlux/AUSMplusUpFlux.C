...
addToRunTimeSelectionTable(godunovFlux, AUSMplusUpFlux, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

AUSMplusUpFlux::AUSMplusUpFlux
(
...
:
    ...
    MInf_(readScalar(coeffDict_.lookup("MInf")))
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void AUSMplusUpFlux::evaluate
(   
    scalar& rhoFlux,
    vector& rhoUFlux,
    scalar& rhoEFlux,
...
) const
{
    ...
    const scalar sqrMaZero = min(1.0, max(sqrMaDash, sqr(MInf_)));
    ...
    (
        (
            -Kp/fa*max(1.0 - sigma*sqrMaDash, 0.0)*(pRight - pLeft)
        )/(rhoTilde*sqr(aTilde))
    );
    ...
    (
        (
            -Ku*P5alphaPlusLeft*P5alphaMinusRight*(rhoLeft + rhoRight)
    ...
    const scalar pTilde
    (
        pLeft*P5alphaPlusLeft + pRight*P5alphaMinusRight + pU
    );
    ...
    (
        (
          ...
          + pTilde*(dotX & normalVector)
        )*magSf
    );
}


void AUSMplusUpFlux::update()
{
	evaluateFlux();
}
