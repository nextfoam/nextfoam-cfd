...
Foam::tmp<Foam::surfaceScalarField> Foam::fvc::interpolate
(
...
)
{
    ...
    (
        (
            vf.dimensions() == dimPressure
         || vf.dimensions() == dimPressure/dimDensity
        )
     && ( vf.name() == "p" || vf.name() == "p_rgh")
    )
    {
        return interpolatePressure(vf, "interpolate(" + vf.name() + ')');
    }
    else
    {
        return interpolate(vf, "interpolate(" + vf.name() + ')');
...
Foam::tmp<Foam::pressureInterpolationScheme> Foam::fvc::pressureScheme
(
    const fvMesh& mesh,
    const word& name
)
{
    return pressureInterpolationScheme::New
    (
        mesh,
        mesh.interpolationScheme(name)
    );
...
Foam::tmp<Foam::surfaceScalarField> Foam::fvc::interpolatePressure
(
    const volScalarField& vf,
    const word& name
)
{
    ...
    return pressureScheme(vf.mesh(), name)().interpolate(vf);
}
