...
void turbulentTemperatureRadCoupledMixedFvPatchScalarField::updateCoeffs()
{
    ...
    const scalarField qAniso(qAnisoCorr(Tp));


    scalarField TcNbr;
    scalarField TpNbr;
    scalarField KDeltaNbr;
    scalarField qAnisoNbr;
    ...
    if (mpp.sameWorld())
    {
        ...
        qAnisoNbr = nbrField.qAnisoCorr(nbrField);
    }
    else
    {
    ...
    distribute(this->internalField().name() + "_qAniso", qAnisoNbr);
    ...
    else
    {
        ...
        refValue() = TcNbr - (qAniso + qAnisoNbr)/KDeltaNbr;
        refGrad() = (qr + qrNbr)/kappaTp;
    }
