...
void Foam::externalWallHeatFluxTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    switch (mode_)
    {
        case fixedPower:
        {
            tmp<scalarField> heatPower =
                tmp<scalarField>::New
                (
                    Tp.size(),
                    Q_->value(this->db().time().timeOutputValue())
                )
              + qAnisoCorr(Tp)/gSum(patch().magSf());
        ...
        case fixedHeatTransferCoeff:
        {
            ...
            const tmp<scalarField> anisoRefValue 
                = qAnisoCorr(Tp)*(1.0/kappaDeltaCoeffs + 1.0/hp);
            ...
            forAll(Tp, i)
            {
                if (qr[i] < 0)
                {
                    ...
                    refValue()[i] = hpTa[i]/hpmqr + anisoRefValue()[i];
                    valueFraction()[i] = hpmqr/(hpmqr + kappaDeltaCoeffs[i]);
                }
                else
                {
                    refValue()[i] = 
                        (hpTa[i] + qr[i])/hp[i] + anisoRefValue()[i];
