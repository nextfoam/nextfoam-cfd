...
)
{
    ...
    readBoundaryFields(); //by Gill

    if (this->name() == "U" && typeName == "volVectorField") // by Gill
    {
...
template<class Type, template<class> class PatchField, class GeoMesh>
void Foam::GeometricField<Type, PatchField, GeoMesh>::readBoundaryFields()
{
    IOobject fieldBfIO
    (
        this->name(),
        this->instance(),
        this->local()/"boundaryFields",
        this->db(),
        IOobject::MUST_READ,
        IOobject::NO_WRITE,
        false
    ...
    const word& fieldName(this->name());

    regExp ILambda("ILambda.*");

    if
    (
        fieldBfIO.template typeHeaderOk
        <
            GeometricField<Type, PatchField, GeoMesh>
        >(true)
    )
    {
        const localIOdictionary bDict(fieldBfIO, typeName);

        boundaryField_.readField(*this, bDict.subDict("boundaryField"));
    }

    // Special treatment for fvDOM
    if (ILambda.match(fieldName))
    {
        IOobject IDefaultBfIO
        (
            "IDefault",
            this->instance(),
            this->local()/"boundaryFields",
        ...
        if
        (
            IDefaultBfIO.template typeHeaderOk
            <
                GeometricField<Type, PatchField, GeoMesh>
            >(true)
        )
        {
            localIOdictionary IDefaultBDict(IDefaultBfIO, typeName);

            dictionary& bDict(IDefaultBDict.subDict("boundaryField"));
            forAllIter(dictionary, bDict, iter)
            {
                iter().dict().remove("value");
                iter().dict().remove("refGradient");
                iter().dict().remove("valueFraction");
            }
            ...
            forAll(boundaryField_, patchi)
            {
                const word& patchName = boundaryField_[patchi].patch().name();

                OStringStream oBuf;
                oBuf << boundaryField_[patchi];
                IStringStream iBuf(oBuf.str());
                dictionary patchDict(iBuf);

                if (bDict.found(patchName))
                {
                    patchDict.merge(bDict.subDict(patchName));
                }

                newBDict.add(patchName, patchDict);
            }

            boundaryField_.readField(*this, newBDict);
        }
    }
} // by Gill
...
template<class Type, template<class> class PatchField, class GeoMesh>
void Foam::GeometricField<Type, PatchField, GeoMesh>::writeBoundaryFields
(
    const bool writeOnZero
) const
{
    bool writeBoundaryField(true);
    bool writeAbsolutePressure(false);

    // Temporary treatment for absolute pressure
    if 
    (
        this->name() == "p"
     && this->dimensions() == dimPressure
    )
    {
        writeAbsolutePressure = true;
    }

    if 
    (
        this->time().timeName() == "0" 
     && !writeOnZero
     && !this->time().processorCase()
    )
    {
        writeBoundaryField = false;
    }
    ...
    )
    {
        ...
        mkDir(fieldBfIO.path());

        OFstream bfOs
        (
            fieldBfIO.objectPath(),
            //this->time().writeFormat(),
            IOstream::ASCII,
            //IOstream::currentVersion,
            this->time().writeCompression()
        );

        fieldBfIO.writeHeader(bfOs, typeName);
        this->boundaryField().writeEntry("boundaryField", bfOs);
        fieldBfIO.writeEndDivider(bfOs);
    }
} // by Gill
...
bool Foam::GeometricField<Type, PatchField, GeoMesh>::readOldTimeIfPresent()
{
    ...
    {
        DebugInFunction
            << "Reading old time level for field" << nl 
            << this->info() << endl;
        ...
        // Note: required for backwards compatibility in case of restarting 
        // from an old run where the oriented state may not have been set
...
writeData(Ostream& os) const
{
    ...
    writeBoundaryFields(); // by Gill

    if (this->name() == "U" && typeName == "volVectorField") // by Gill
    {
        if 
        (
            !this->mesh().thisDb().objectRegistry::template 
            foundObject
            <
...
template<class Type, template<class> class PatchField, class GeoMesh>
void Foam::GeometricField<Type, PatchField, GeoMesh>::writeBoundaryField
(
    const bool writeOnZero
)
{
    ...
    writeBoundaryFields(writeOnZero);
} // by Gill
