...
Foam::fileName Foam::argList::procInfoDirName_ = ".foamProcInfo";
Foam::fileName Foam::argList::procInfoFileName_ = "procInfo";
...
)
{
    ...
    if (initialise)
    {
        ...
        fileName procInfoFile;
        if (Pstream::master())
        {
            procInfoFile = 
                rootPath_/globalCase_/procInfoDirName_/procInfoFileName_;
        }

        // Switch on signal trapping. We have to wait until after Pstream::init
        // since this sets up its own ones.
        #if defined(__linux__) || defined(__APPLE__)
            sigFpe::set(procInfoFile, bannerEnabled());
            sigAbrt::set(bannerEnabled());
            sigTerm::set(bannerEnabled());
            sigUsr1::set(bannerEnabled());
        #else
...
void Foam::argList::createProcInfoFile() const
{
    if (Pstream::master())
    {
        Foam::mkDir(rootPath_/globalCase_/procInfoDirName_);
        const string dateString = clock::date();
        const string timeString = clock::clockTime();

        OFstream os(rootPath_/globalCase_/procInfoDirName_/procInfoFileName_);

        dictionary procInfoDict;
        procInfoDict.add("Build", foamVersion::build, true);
        procInfoDict.add("API", foamVersion::api, true);
        procInfoDict.add("Version", foamVersion::version.c_str(), true);
        procInfoDict.add("Arch", foamVersion::buildArch, true);
        procInfoDict.add("Exec", executable_, true);
        procInfoDict.add("Command", commandLine_, true);
        procInfoDict.add("Date", dateString, true);
        procInfoDict.add("Time", timeString, true);
        procInfoDict.add("Host", hostName().c_str(), true);
        procInfoDict.add("User", userName().c_str(), true);
        procInfoDict.add("PID", pid(), true);
        procInfoDict.add("Case", rootPath_/globalCase_, true);
        procInfoDict.add("nProcs", Pstream::nProcs(), true);
        procInfoDict.add("State", "running", true);

        if (os.good())
        {
            procInfoDict.writeEntries(os, false);
        }
    }
}
