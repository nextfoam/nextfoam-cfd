...
) const
{
    ...
    if (foundObject<fieldType>(fieldName))
    {
        ...
        typename pTraits<Type>::labelType validPerfComponents
        (
            mesh_.validPerfComponents<Type>()
        );

        for (direction cmpt=0; cmpt<pTraits<Type>::nComponents; ++cmpt)
        {
            if (component(validPerfComponents, cmpt) != -1)
            {
...
void Foam::functionObjects::solverInfo::updateSolverInfo(const word& fieldName)
{
    ...
    if (foundObject<volFieldType>(fieldName))
    {
        ...
        if (solverDict.found(fieldName))
        {
            ...
            const labelType validPerfComponents(mesh_.validPerfComponents<Type>());
