...
) const
{
    ...
    bool writeBoundaryField(false); // by Gill
    ...
    forAll(bf, patchi)
    {
        ...
        const word& polyPatchType(bf[patchi].patch().type()); // by Gill
    ...
    if (writeBoundaryField) // by Gill
    {
        result.readOpt() = IOobject::MUST_READ;
        result.writeOpt() = IOobject::AUTO_WRITE;
    }
