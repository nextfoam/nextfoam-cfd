...
bool Foam::basicThermo::isASolid_(false);
...
Foam::volScalarField& Foam::basicThermo::lookupOrConstructPressure
(
    const fvMesh& mesh,
    bool& isOwner
)
{
    auto* ptr = mesh.objectRegistry::getObjectPtr<volScalarField>("p");
    ...
    if (!ptr)
    {
        ...
        (
            mesh.time().foundObject<uniformDimensionedVectorField>("g")
         && mesh.foundObject<IOdictionary>("operatingConditions")
        )
        {
            const IOdictionary& operatingConditions
            (
                mesh.lookupObject<IOdictionary>("operatingConditions")
            );
...
:
    ...
    includeViscousDissipation_
    (
        getOrDefault<Switch>("includeViscousDissipation", false)
    ),
    includeKineticEnergy_(getOrDefault<Switch>("includeKineticEnergy", false)),
    includePressureWork_(getOrDefault<Switch>("includePressureWork", true)),

    //p_(lookupOrConstruct(mesh, "p", pOwner_)),
    p_(lookupOrConstructPressure(mesh, pOwner_)), // by Gill
...
{
    if (includeViscousDissipation_)
    {
        includeKineticEnergy_ = true;
        includePressureWork_ = true;
    }
